...
const Productos = () => {
  ...
  const idMarca = params.idMarca ? parseInt(params.idMarca) : null;
  ...
  useEffect(() => {
    if (idMarca && !isNaN(idMarca)) {
      const filtrados = celulares.filter(celular => celular.marcaId === idMarca);
      setFilteredProducts(filtrados);
      setActiveMarca(idMarca);
    } else {
  ...
  <img 
    src={celular.fotos[0]} 
    alt={celular.nombre}
    className={`producto-image ${celular.marcaId === 1 ? 'iphone' : ''}`}
    onError={(e) => {
  ...
  />
              ...
              <div className="producto-content">
                ...
                <p className="producto-brand">
                  {marcas.find(m => m.id === celular.marcaId)?.nombre}
                </p>
